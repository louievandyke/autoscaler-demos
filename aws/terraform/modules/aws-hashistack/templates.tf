...
/*
data "template_file" "efs_plugin_jobspec" {
  template = file("${path.module}/templates/plugin-efs.nomad.tpl")
...
resource "null_resource" "efs_plugin_jobspec" {
  depends_on = [nomad_namespace.hashistack]  
  provisioner "local-exec" {
    command = "echo '${data.template_file.efs_plugin_jobspec.rendered}' > efs-plugin.nomad"
  }
}
*/
